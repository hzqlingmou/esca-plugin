...
export class Update extends plugin {
    ...
    async runUpdate(isForce) {
        ...
        if (ret.error) {
            logger.mark(`${this.e.logFnc} 更新失败：逸燧插件`)
            this.gitErr(ret.error, ret.stdout)
            return false
        }
        /** 获取插件提交的最新时间 */
        let time = await this.getTime('esca-plugin')

        if (/(Already up[ -]to[ -]date|已经是最新的)/.test(ret.stdout)) {
            await this.reply(`逸燧插件已经是最新版本\n最后更新时间：${time}`)
        } else {
            await this.reply(`逸燧插件\n最后更新时间：${time}`)
            this.isUp = true
            /** 获取逸燧插件的更新日志 */
            let log = await this.getLog('esca-plugin')
    ...
    async getLog(plugin = '') {
        ...
        let end = '更多详细信息，请前往github查看\nhttps://github.com/hzqlingmou/esca-plugin'
